...
July2020_df =
  ...
  tibble(year_month = "July2020")

July2024_df =
  read_csv("./data/CitiBike/July 2024 citi.csv", na = c("NA", ".", "")) |>
  tibble(year_month = "July2024")
...
Before merging, `year_month` variable is created for each individual dataset. After merging the 4 datasets, the resulting data is stored in `merged_bikes` (`r nrow(merged_bikes)` rows x `r ncol(merged_bikes)` columns).
